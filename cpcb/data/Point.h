...
public:
  ...
  Dim distanceToOrigin() const { return Dim::quadrature(x, y); }
  bool isNull() const;
  ...
  Point &freeRotate(class FreeRotation const &degCW, Point const &p=Point());
  Point rotatedFreely(FreeRotation const &degCW, Point const &p=Point()) const;
