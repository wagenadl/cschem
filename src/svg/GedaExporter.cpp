...
public:
  GedaExporterData(Circuit const &circ, SymbolLibrary const *lib):
    circ(circ), lib(lib),
    geom(circ, lib) {
  }
...
void GedaExporterData::writeConnection(QXmlStreamWriter &sw,
...
GedaExporter::GedaExporter(Circuit const &circ, SymbolLibrary const *lib):
  d(new GedaExporterData(circ, lib)) {
}
